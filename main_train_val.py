...
from BERT_modeltraining_Imon_orig import modeltraining, f1_score_func, evaluate, data_split, tokenizationBERT
from ClinicalBERT_training_original_imon import clinicalmodeltraining
import argparse
...
def one_round_train(seed_val, df, model_name, data_path, client_names):
    ...
    imon_dataset_train, imon_dataset_val, imon_dataset_test = tokenizationBERT(imon_df) # impression+finding
    batch_size = 5
    ...
    for epoch in tqdm(range(1, epochs + 1)):
        ...
        for dataset_train, dataset_val, name in zip(train_datasets, val_datasets, client_names):
            ...
            if val_f1 > clients_val_f1[name]:
                ...
                torch.save(model.state_dict(), './models/standard/' + model_name + name + '_seed' + str(seed_val) + '.pth')
...
if __name__ == "__main__":
    ...
    CWT_train = False
    client_names = ['imon']
    model_name = 'imon_findings+impressions_'
